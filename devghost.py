...
def setup():
    ...
    github_token = click.prompt('Enter your GitHub Personal Access Token', hide_input=True)
    ...
    try:
        try:
            ...
            if 'unknown revision or path not in the working tree' in output or 'ambiguous argument' in output:
                # No commits yet, get all changes
        ...
        if not diff.strip():
            ...
            return
        # Truncate diff if too large for Gemini
        max_diff_len = 4000
...
"""
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                # Offer to edit the message
                if click.confirm('Would you like to edit the commit message?', default=False):
                    edited = click.edit(message)
                    if edited:
                        message = edited.strip()
                # Offer to commit
                if click.confirm('Would you like to commit with this message?', default=False):
                    try:
